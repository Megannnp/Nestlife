...
**macOS 用户**：解压完，直接**双击 `install.command`**（就是交付包里的那个文件），它会自动：
...
全程 3~5 分钟，不用敲一条命令（除了可能让输一次电脑开机密码装 Node）。

> ✅ 看到「🎉 安装完成」+ 浏览器自动打开就是成功了 → 直接跳到第 5 节。
> 生成的登录密码保存在 `~/.nestlife-password.txt`，手机访问时用。
>
> **Windows 用户**：没有一键脚本，用 Docker Desktop（见第 4 节方式一），也很快。
