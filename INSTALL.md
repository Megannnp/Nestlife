...
## 2.5 ⭐ 最快路径：一键安装（推荐，自动完成后面所有步骤）

解压完，**双击对应你系统的文件**：

| 系统 | 双击这个 | 效果 |
|---|---|---|
| **macOS** | `install.command` | 自动装 Node → 装依赖 → 构建 → 生成密码 → 启动 → 打开浏览器 |
| **Windows** | `install.bat` | 同上（自动装 Node / 生成密码 / 启动 / 打开浏览器） |
...
全程 3~5 分钟，不用敲一条命令（macOS 装 Node 时可能要输一次开机密码）。

> ✅ 看到「安装完成」+ 浏览器自动打开就是成功了 → 直接跳到第 5 节。
> 生成的登录密码保存在 `~/.nestlife-password.txt`（Windows 在 `%USERPROFILE%\.nestlife-password.txt`），手机访问时用。
