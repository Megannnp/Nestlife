...
效果：
- 未登录访问 `/api/*` 返回 401；访问页面自动跳转 `/login`。登录态 30 天有效。
- **本机（`localhost`）访问免登录**：用户本机浏览器、本机 OpenClaw(Mira) 的 API 调用均不受影响；
  局域网/公网来源（手机、其他设备）必须登录后才能访问个人数据。
- 手机/其他设备访问：浏览器打开 `http://<服务器IP>:3100` → 输入密码登录。
- 注意：免登录按请求 `Host` 头判断，可防止"随手访问"，但同网段攻击者伪造 Host 头可绕过，
  公网部署建议同时启用防火墙仅放行已知来源。
...
**现在不需要 OpenClaw 也能"对话即执行"了**：NestLife 内置执行器（function calling + 白名单工具），任意支持工具调用的网关（DeepSeek / Ollama 等）都能：
- 「XX 完成了」→ 自动完成对应任务
- 「加个任务：周五前写方案」→ 自动新增任务
- 「查一下今天有什么安排」→ 列出今日任务
- 「我们决定用方案B」→ 记录到决策室
- 「搜一下知识库里关于XX的内容」→ 检索知识库

配置即用（无需额外安装）：
1. **设置 → AI 助手接入** → 填 DeepSeek 或 Ollama 网关
2. 保存即生效——内置执行器自动启用

若需要**更完整的能力**（读任意文件、执行命令、MCP 插件），再装 OpenClaw：
1. 安装并启动 OpenClaw（`npm install -g openclaw`，详见官方文档）
2. **设置 → AI 助手接入** → 「⚡ 一键接入」→ 自动切换到 OpenClaw 完整执行模式
